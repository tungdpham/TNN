...
public:
  ...
  explicit DivLayer(const std::string &name = "div") { this->name_ = name; }
  ...
  graph_api_v2::Node operator()(const graph_api_v2::Node &a, const graph_api_v2::Node &b) {
    if (!a || !b) {
      throw std::runtime_error("DivLayer: input nodes cannot be null");
    }
    graph_api_v2::Graph *graph = a->graph();
    if (graph != b->graph()) {
      throw std::runtime_error("DivLayer: both input nodes must belong to the same graph");
    }
